...
1. **Detect the stack.** Read the real manifests and the top-level folder list (never build output; see "Where to look" in the registry), then grep the source for the registry's source signals. Match against `references/service-registry.md`. Output three things before doing anything else: the stack table (one row per module, all providers in the cell), the package manager, and the list of reviewers to activate. Show all three in chat. If `audit.md` or `audit-brief.md` already exist in the project root, say so and ask whether to overwrite or write `audit-<date>.md` instead; default to overwrite only if the user says so.

2. **Pre-flight plugin check.** For each detected service with a plugin id, check `~/.claude/plugins/installed_plugins.json`. A plugin counts as installed if it has a `user` scope entry, or a `project`/`local` scope entry whose `projectPath` is the project being audited; an entry scoped to a different project does not count. Re-read the file on resume. Print one line per service: installed ✅ or missing ❌ with the exact install commands from the registry. If anything is missing, say "Install the ones you want, then reply *continue* (or *continue without*)." and **stop the turn**. Do not start step 3 in the same turn. On resume, re-check installs, then proceed.
