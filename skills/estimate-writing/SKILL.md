...
7. **Write `estimate.md`** from `templates/estimate.md` next to the brief. If `estimate.md` already exists there, ask whether to overwrite or write `estimate-<date>.md`; in a non-interactive run write the dated file. "Basis" names the actual brief file and the phases estimated (for example "Phases 1 to 3; Phase 4 (mobile) is listed under Not included"). Fill "What is included" with one sentence a non-technical reader understands: what gets done and why it matters.
